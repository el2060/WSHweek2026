...
- Scenario 02 uses `src/fire.css` for a full-width route-photo canvas. All 14 original photos remain, with visible thumbnails and a short location caption for alternate views. The former left instruction panel is removed. One compact panel now combines the checkpoint, a neutral situation, two choices and answer-specific feedback. On phones/tablets, the photo stage leads into this panel; text grows naturally without inner scrollboxes.
- Fine-pointer desktops retain gentle photo-only parallax (up to 9px horizontally / 6px vertically, with a slight tilt). Text/buttons stay still. Touch devices, smaller screens and reduced-motion settings disable the movement. No route photos or map assets are removed or replaced.
- `scripts/verify-fire-immersive.js` checks all 14 photos, both choices at seven checkpoints, blank initial answers/feedback, gallery keyboard use, full-width imagery, non-overlapping panels, completion, route map, mobile/enlarged text and parallax/reduced-motion behaviour.
- Block 56 is a **stay-on-the-walkway checkpoint, not a crossing** (route-owner guidance). A single prominent warning remains in the question panel. The decision asks how to respond to a colleague heading towards that crossing; feedback explains the route distinction. The later Admin Field crossing has its own decision about crossing safely under the warden's direction.
