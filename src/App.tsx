...
import { ArrowDown, ArrowRight, BookOpen, Building2, Check, ClipboardCheck, CloudFog, ExternalLink, Eye, Flame, Footprints, HeartHandshake, Info, Menu, Phone, Sparkles, Volume2, VolumeX, Wrench, X } from 'lucide-react';
...
function Intro({ onStart, startLabel }: { onStart: () => void; startLabel: string }) {
  return <section id="intro" className="intro">
    <div className="intro-copy">
      ...
      <h1>CLTE Workplace Safety <em>Activity</em></h1>
      <p className="tagline">Complete five short scenarios based on situations you may encounter at work.</p>
...
function OfficeScene({ onComplete }: { onComplete: () => void }) {
  ...
  return <section id="office" className="chapter">
    ...
    {handled.length===officeHotspots.length&&<div className="scene-complete reveal"><Check/><strong>Office hazards addressed</strong><button className="primary" onClick={onComplete}>Continue <ArrowRight/></button></div>}
  </section>;
}
  ...
  return <section id="evacuation" className={`chapter evacuation fire-response fire-stage-${fireStage} ${activeChoice?.best?'decision-safe':''}`}>
    ...
    {mapOpen&&<div className="assembly assembly-point-drill reveal">
      <div className={`assembly-marker ${destinationCorrect?'confirmed':''}`} aria-live="polite">
        {destinationCorrect?<><div className="assembly-pin"><span>A</span></div><p>CLTE assembly point</p><h3>{officialInfo.assemblyArea}</h3><strong>{officialInfo.assemblyZone}</strong><small>Follow fire wardens and posted instructions.</small></>:<><div className="assembly-pin"><span>?</span></div><p>Assembly point check</p><h3>Choose a location</h3><small>Select an answer to reveal the CLTE assembly point.</small></>}
      </div>
...
export default function App() {
  ...
  return <div className="app-shell"><header><button className="logo" onClick={()=>setView('intro')} aria-label="Ngee Ann Polytechnic · CLTE workplace safety activity · Home"><img src="/assets/np-logo.png" alt="Ngee Ann Polytechnic"/><span className="logo-title">CLTE staff activity</span></button><nav className={menu?'open':''}>{chapters.map(({n,id,label})=><button key={id} disabled={!canVisit(id)} onClick={()=>setView(id)} className={`${view===id?'current':''} ${progress[id]?'done':''}`}>{n}<span>{label}</span></button>)}</nav><div className="header-tools"><span>{completed}/5 scenarios</span><button onClick={()=>setSound(!sound)} aria-label={sound?'Turn sound off':'Turn sound on'}>{sound?<Volume2/>:<VolumeX/>}</button><button className="menu" onClick={()=>setMenu(!menu)} aria-label="Toggle navigation">{menu?<X/>:<Menu/>}</button></div></header>
    <main className="experience-stage"><div key={view} className="view-transition">{view==='intro'&&<Intro startLabel={progress.completion?'Review activity':completed?'Continue activity':'Start activity'} onStart={()=>setView(resumeView)}/>} {view==='office'&&<OfficeScene onComplete={()=>complete('office','walkway')}/>} {view==='walkway'&&<WetScene onComplete={()=>complete('walkway','haze')}/>} {view==='haze'&&<HazeInterstitial onComplete={()=>complete('haze','evacuation')}/>} {view==='evacuation'&&<EvacuationScene onComplete={()=>complete('evacuation','reporting')}/>} {view==='reporting'&&<RoutingScene onComplete={()=>complete('reporting','practice')}/>} {view==='practice'&&<PracticeReport onComplete={()=>complete('practice','guide')}/>} {view==='guide'&&<PocketGuide onComplete={()=>{setProgress(value=>({...value,guide:true,completion:true}));setView('completion')}}/>} {view==='completion'&&<Completion onReview={()=>setView('office')} onGuide={()=>setView('guide')}/>}</div></main></div>;
}
