...
export default function PracticeReport({ onComplete }: { onComplete: () => void }) {
  ...
  const choices = (label: string, key: keyof typeof data, items: string[]) => <fieldset className="choice-field"><legend>{label}</legend><div>{items.map(item => <button type="button" key={item} aria-pressed={data[key] === item} className={data[key] === item ? 'selected' : ''} onClick={() => choose(key, item)}>{data[key] === item && <Check/>}<span>{item}</span></button>)}</div><details className="practice-hint"><summary>Hint</summary><p className="practice-choice-cue"><ReadingText>{choiceHints[key]}</ReadingText></p></details></fieldset>;
  ...
  return <section className="practice mock-report" id="practice">
    <div className="simulation-banner"><Info/><span><strong>Guided practice</strong> · Nothing is submitted.</span></div>
    <div className="practice-case"><strong>Case</strong><p><ReadingText>On 27 April, a student slips on the wet walkway beside Block 73 and has a minor injury. You check on them, keep others clear and arrange first-aid support.</ReadingText></p><span>Use the case details. No names or contact details are needed.</span></div>
    <div className="mock-shell">
      <aside className="mock-steps"><p className="eyebrow">Practice report</p><h3>Wet walkway incident</h3>{['Type', 'Impact', 'Details'].map((label, index) => <button key={label} aria-current={step === index ? 'step' : undefined} onClick={() => changeStep(index)} className={`${step === index ? 'current' : ''} ${stepReady[index] ? 'done' : ''}`}><span>{stepReady[index] ? <Check/> : index + 1}</span>{label}</button>)}</aside>
