...
function MomentNav({ labels, step, done, onChange }: { labels: string[]; step: number; done: boolean[]; onChange: (step: number) => void }) {
  return <div className="moment-navigation">
    ...
    <p>Three small actions. No score. Explore any moment.</p>
  </div>;
}
...
function HelpPractice({ emergency, done, onAct }: { emergency: boolean; done: boolean; onAct: () => void }) {
  return <div className="help-practice">
    <div className="practice-recipient"><Phone size={22}/><div><span>{emergency ? 'Emergency ambulance' : 'Nearby colleague / first aider'}</span><strong>{emergency ? officialInfo.ambulanceNumber : 'Ask for first-aid support'}</strong></div><span className="practice-label">Practice only</span></div>
    <p className="script-label">Here’s what you could say</p>
    ...
    <p className="practice-disclaimer">No call is made. Nothing is recorded or sent.</p>
  </div>;
}
  ...
  return <section className="guided-scene haze-guided" id="haze">
    ...
    <div className="guided-workspace">
      ...
      <div className="moment-panel">
        <div className="moment-body" key={step}>
          <p className="eyebrow">Moment 0{step + 1}</p><h2>{titles[step]}</h2>
          {step === 0 && <><p className="moment-cue">Don’t push on with an outdoor activity when someone feels unwell. Move it indoors or postpone it — either works here.</p>
